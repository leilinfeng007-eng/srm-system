...
test('internal and supplier OpenAPI contracts remain explicitly separated', () => {
  ...
  assert.ok('/api/v1/master-data/organizations' in internal.paths)
  assert.ok('/api/v1/system/approvals' in internal.paths)
  assert.ok('/api/v1/system/inbox-events' in internal.paths)
  assert.ok(Object.keys(internal.paths).length >= 100)
