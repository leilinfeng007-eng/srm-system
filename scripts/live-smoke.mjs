...
check(adminMenus.response.status === 200 && adminMenus.body.data.length === 12, '管理员一级菜单数量不正确')
check(adminPages === 86, '管理员可访问页面数量不正确')
...
check(viewerMenus.response.status === 200 && viewerMenus.body.data.length === 2, '查看者菜单过滤失败')
