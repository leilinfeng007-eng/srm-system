...
interface ParamItem { id:number; paramCode:string; paramName:string; paramType:string; defaultValue:string; approvalRequired:boolean; description:string }
interface ParamVersion { id:number; version:number; paramValue:string; status:string; publishedAt:string }
...
const paramDialog=ref(false); const paramForm=reactive({id:null as number|null,paramCode:'',paramName:'',paramType:'STRING',defaultValue:'',approvalRequired:false,description:''})
const versionDialog=ref(false); const versions=ref<ParamVersion[]>([]); const newValue=ref(''); const currentParamId=ref<number|null>(null)
...
async function loadDicts() { loading.value=true; try{ const r=await internalApi.get<DictItem[]>('/system/dictionaries'); dictList.value=r??[] } catch(e){ void e; dictList.value=[] } finally { loading.value=false } }
...
function openParamCreate(){Object.assign(paramForm,{id:null,paramCode:'',paramName:'',paramType:'STRING',defaultValue:'',approvalRequired:false,description:''});paramDialog.value=true}
...
async function toggleItem(row:DictionaryItem,action:'enable'|'disable'){if(!currentDictionary.value)return;try{await internalApi.post('/system/dictionaries/'+currentDictionary.value.id+'/items/'+row.id+'/'+action);await openItems(currentDictionary.value);ElMessage.success('操作成功')}catch(e){void e}}
...
async function toggleNumber(row:NumberRule,action:'enable'|'disable'){try{await internalApi.post('/system/number-rules/'+row.id+'/'+action);loadNumbers();ElMessage.success('操作成功')}catch(e){void e}}

async function saveDict(){ try{ if(!dictForm.id){ await internalApi.post('/system/dictionaries',dictForm) } else { await internalApi.request('/system/dictionaries/'+dictForm.id,{method:'PUT',body:dictForm}) } dictDialog.value=false; loadDicts(); ElMessage.success('保存成功') } catch(e){ void e; ElMessage.error((e as Error)?.message||'保存失败') } }
async function toggleDict(row:DictItem,action:string){ try{ await internalApi.post('/system/dictionaries/'+row.id+'/'+action); loadDicts(); ElMessage.success('操作成功') } catch(e){ void e; } }

async function saveParam(){ try{ if(!paramForm.id){ await internalApi.post('/system/parameters',paramForm) } paramDialog.value=false; loadParams(); ElMessage.success('保存成功') } catch(e){ void e; ElMessage.error((e as Error)?.message||'保存失败') } }
async function openVersions(row:ParamItem){ currentParamId.value=row.id; newValue.value=''; try{ const r=await internalApi.get<ParamVersion[]>('/system/parameters/'+row.id+'/versions'); versions.value=r??[] } catch(e){ void e; versions.value=[] } versionDialog.value=true }
async function addVersion(){ if(!currentParamId.value) return; try{ await internalApi.post('/system/parameters/'+currentParamId.value+'/versions',{paramValue:newValue.value}); ElMessage.success('已创建草稿版本'); openVersions({id:currentParamId.value} as ParamItem) } catch(e){ void e; ElMessage.error((e as Error)?.message||'创建失败') } }
async function submitVersion(row:ParamVersion){ if(!currentParamId.value) return; try{ await internalApi.post('/system/parameters/'+currentParamId.value+'/versions/'+row.id+'/submit'); ElMessage.success('已提交审批'); openVersions({id:currentParamId.value} as ParamItem) } catch(e){ void e; ElMessage.error((e as Error)?.message||'提交失败') } }
...
<template>
  <div style="padding:16px">
    <el-card>
      <el-tabs v-model="activeTab" @tab-change="onTab">
        <el-tab-pane label="字典" name="dict">
          <div class="bar"><b>字典列表</b><el-button v-if="canCreateDict" type="success" size="small" style="margin-left:auto" @click="openDictCreate">新增字典</el-button></div>
          <el-table :data="dictList" v-loading="loading" border size="small" style="margin-top:10px" empty-text="暂无字典">
            ...
            <el-table-column label="操作" width="190"><template #default="{row}">
              ...
              <el-button v-if="canUpdateDict" link size="small" @click="dictDialog=true;Object.assign(dictForm,{id:row.id,dictCode:row.dictCode,dictName:row.dictName,description:row.description})">编辑</el-button>
        ...
        <el-tab-pane label="参数" name="param">
          ...
          <el-table :data="paramList" v-loading="loading" border size="small" style="margin-top:10px" empty-text="暂无参数">
            ...
            <el-table-column label="操作" width="100"><template #default="{row}"><el-button link type="primary" size="small" @click="openVersions(row)">版本</el-button></template></el-table-column>
          </el-table>
        </el-tab-pane>
      </el-tabs>
    </el-card>
    ...
    <el-dialog v-model="paramDialog" :title="'新增参数'" width="480px">
      <el-form label-width="90px"><el-form-item label="编码"><el-input v-model="paramForm.paramCode"/></el-form-item><el-form-item label="名称"><el-input v-model="paramForm.paramName"/></el-form-item><el-form-item label="类型"><el-select v-model="paramForm.paramType"><el-option label="字符串" value="STRING"/><el-option label="整数" value="INTEGER"/><el-option label="小数" value="DECIMAL"/><el-option label="布尔" value="BOOLEAN"/><el-option label="日期" value="DATE"/></el-select></el-form-item><el-form-item label="默认值"><el-input v-model="paramForm.defaultValue"/></el-form-item><el-form-item label="需审批"><el-switch v-model="paramForm.approvalRequired"/></el-form-item></el-form>
    ...
    <el-dialog v-model="versionDialog" title="参数版本" width="560px">
      ...
      <el-table :data="versions" border size="small">
        ...
        <el-table-column label="操作" width="90"><template #default="{row}"><el-button v-if="canSubmitParam&&row.status==='DRAFT'" link type="primary" @click="submitVersion(row)">提交审批</el-button></template></el-table-column>
      </el-table>
